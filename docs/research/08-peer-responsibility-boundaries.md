...
replicas³³. An in-memory-only breaker is near-useless across more than one replica, the same
hazard as the scheduler. Measured again in September 2026, purgatory had not been committed to in about
twenty-two months, which is why
[ADR-0057](../adr/0057-reliability-middlewares-and-error-reporting-stay-outside.md) recommends no
breaker library at all ([`docs/research/26`](26-schedule-reliability-and-probe-primitives.md) §6).
...
|---|---|---|
| (a) cron/interval scheduling | **Recipe** — [ADR-0056](../adr/0056-the-framework-owns-no-scheduler.md) decided no scheduler at all, not even a plugin, on the maintenance evidence of [`docs/research/26`](26-schedule-reliability-and-probe-primitives.md) | Every peer that owns scheduling ships it as a separately versioned companion (`taskiq-faststream`, `litestar-saq`) or a gated extra (PTB `[job-queue]`), never core¹²·¹³·³ — and it carries the single-instance-only hazard aiommbot already knows about. |
...
| (b) tracing (OpenTelemetry) | **Recipe** — [ADR-0051](../adr/0051-first-party-observability-plugin.md) decided an extra instead, because auto-instrumentation cannot produce a dispatch span at all and names an HTTP span by method alone | No peer core or extra reimplements OTel; the zero-code auto-instrumentation path already covers HTTP/Mongo clients aiommbot uses²⁴. |
| (c) CLI runner | **Extra** — [ADR-0058](../adr/0058-the-command-is-a-console-script-behind-the-click-extra.md) decided a two-command console script behind the `click` extra | Every framework that has one keeps it to `run`/`dev`/discovery on top of click or Typer, not a bespoke parser¹⁸·¹⁹·²⁰ — this is wiring, not infrastructure, and it's the one thing every consumer needs identically. |
...
| (e) idempotency/dedup | **Plugin** — [ADR-0060](../adr/0060-flood-control-and-delivery-dedup-are-one-generic-plugin.md) decided one generic Plugin for delivery dedup and flood control together | Peers treat this as a precondition of `acks_late`/at-least-once delivery, never automated³² — but aiommbot's specific dedup case (Mattermost webhook replay, duplicate action callbacks) is a domain concern, see §6. |
...
| (g) circuit breaker | **Recipe, naming no library** — [ADR-0057](../adr/0057-reliability-middlewares-and-error-reporting-stay-outside.md) decided a documented `HTTPTransport` decorator and no recommendation, because no maintained asyncio breaker has shared state ([`docs/research/26`](26-schedule-reliability-and-probe-primitives.md) §6) | purgatory's own value (Redis-backed shared state across replicas) only matters if wired consistently; keeping it a documented DI-injected dependency rather than framework code lets it be swapped/upgraded independently, matching how *every* peer treats circuit breaking (absent from framework core, always a separate library)³³. |
| (h) rate limiting — outbound (Mattermost API) | **Recipe** — [ADR-0026](../adr/0026-standalone-typed-api-client-over-an-http-transport-protocol.md) reads `Retry-After` and `X-RateLimit-Reset` and rejected a client-side bucket; where a limit must hold across replicas [ADR-0057](../adr/0057-reliability-middlewares-and-error-reporting-stay-outside.md) names `PyrateLimiter`, because `aiolimiter` is single-loop by construction | Same shape as Slack's `RateLimitErrorRetryHandler`⁴ — an SDK-adjacent concern, not a framework concern; wrap `AsyncLimiter` around the API client's `HTTPTransport`. |
| (h) rate limiting — inbound (chat flood control) | **Plugin** — [ADR-0060](../adr/0060-flood-control-and-delivery-dedup-are-one-generic-plugin.md) decided the `Cooldown` Flag and an Inbound middleware, which is the shape this row asked for | This is closer to Litestar's `RateLimitConfig` case²³ — genuinely bot-specific (per-user/per-channel cooldown, not per-IP) — see §6 for why it isn't a clean "just extra a library" case. |
...
| (j) error reporting (Sentry) | **Recipe** — [ADR-0057](../adr/0057-reliability-middlewares-and-error-reporting-stay-outside.md) decided no extra: `Failed(error)` reaches Middleware, so the adapter is five lines the application writes | No peer bot framework (aiogram, Bolt, discord.py, hikari, PTB) has an official Sentry integration — a thin first-party adapter is defensible, but it should be a narrow Protocol-shaped adapter around `sentry_sdk`, not a redaction/formatting pipeline of its own. |
...
| (m) health endpoints | **Plugin** — [ADR-0061](../adr/0061-health-is-a-generic-plugin-over-application-supplied-checks.md) decided a generic Plugin serving `/livez` and `/readyz`, so a bot author can add readiness facts of their own ([`docs/research/27`](27-application-contributed-readiness-checks.md)) | FastStream's `make_ping_asgi` is the right size — a tiny helper that pings what the framework already knows how to ping (broker/Mongo/Mattermost connectivity), left for the app to mount¹⁸; a bespoke health *subsystem* would be over-scoped for what every peer treats as "a plain route." |

Every row above whose placement a later decision changed now names that decision in the Placement
cell; a research note records what the sources said and points at the ADR rather than arguing with
it ([`documentation-style.md` §8](../documentation-style.md#8-writing)).
