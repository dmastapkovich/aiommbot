...
**PyMongo's async story is still settling.** `AsyncMongoClient` reached beta in PyMongo 4.9, GA in [4.13](https://pymongo.readthedocs.io/en/stable/changelog.html); Motor's own README states ["As of May 14th, 2025, Motor is deprecated in favor of the GA release of the PyMongo Async API"](https://github.com/mongodb/motor/blob/master/README.md), with bug fixes until 2026-05-14 and critical fixes until 2027-05-14 (the [PyMongo migration page](https://www.mongodb.com/docs/languages/python/pymongo-driver/current/reference/migration/) still words it as "Motor will be deprecated on May 14th, 2026"). aiogram's own migration in [3.22.0](https://github.com/aiogram/aiogram/pull/1705) tracks this exactly — reacting to a driver deprecation, not choosing Mongo's async story as a settled foundation.
...
**A single "storage profile" backend driving state + locks + idempotency together: no precedent found.** Every framework that separates concerns (aiogram, Rasa) uses **two distinct abstractions** — never one object for both, and neither extends to idempotency/DLQ. One storage profile serving all three would not match how any of these frameworks are built. That doesn't make it wrong, but it is a novel design choice to justify on its own merits, not one validated by precedent.

## 6. Recommendation for aiommbot

The decision is [ADR-0022](../adr/0022-state-plugin-model.md): `KeyValueStore` (versioned bytes,
compare-and-set, optional TTL) plus a separate `LockProvider`, with typed `Flow[Data]`.
...
- MongoDB/PyMongo async status: https://github.com/mongodb/motor/blob/master/README.md (Motor deprecated 2025-05-14, bug fixes until 2026-05-14, critical fixes until 2027-05-14); https://www.mongodb.com/docs/languages/python/pymongo-driver/current/reference/migration/; PyMongo changelog https://pymongo.readthedocs.io/en/stable/changelog.html (AsyncMongoClient beta in 4.9, GA in 4.13).
