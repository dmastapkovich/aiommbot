...
| [`36-compose-fields-a-drain-depends-on.md`](36-compose-fields-a-drain-depends-on.md) | Which Docker Compose fields does a graceful drain depend on, what does Compose offer in place of liveness and readiness probes, and can it bound a workload to one live instance? |
| [`41-reporting-which-lifecycle-unit-failed.md`](41-reporting-which-lifecycle-unit-failed.md) | When a lifecycle unit fails, how does the caller learn which one it was, what reaches them when several fail at once, and what does subclassing `ExceptionGroup` cost? |
| [`42-the-lifecycle-contract-of-a-context-manager-unit.md`](42-the-lifecycle-contract-of-a-context-manager-unit.md) | What does a unit driven through `__aenter__`/`__aexit__` see on the way out, may it suppress, is its teardown bounded, and may the host object be run again? |
| [`43-who-owns-a-librarys-logger.md`](43-who-owns-a-librarys-logger.md) | Does a library create its own loggers or take them from the caller, what happens when the caller supplies nothing, and what does either answer cost an operator? |
| [`44-keeping-a-documented-field-list-in-step-with-the-code.md`](44-keeping-a-documented-field-list-in-step-with-the-code.md) | Where does a project write the field names of a configuration object for a reader, and what fails when that document and the code disagree? |
