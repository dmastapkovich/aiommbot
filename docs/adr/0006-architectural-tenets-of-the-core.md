...
2. **Dependency inversion through Core-owned Protocols.** The Core defines the `Protocol`s (the
   Transport, `KeyValueStore`, `LockProvider`, `HTTPTransport`, the `RequestObserver` pair that is
   the Observability seam); adapters and plugins implement them; nothing in the Core imports an
   implementation.
...
6. **Declarative thin handlers.** Filters, state gates, payload parsing and dependency injection
   ...
   exceptions propagate to the ErrorBoundary and reach Middleware as a typed `Failed`
   ([ADR-0048](0048-observability-is-not-a-core-seam.md)); handlers, filters and events are
...
- **A Protocol expresses a seam**; the sixteen Protocols on the thirteen seam rows of the
  building-block view §5.1.1 are the only substitution points, and nothing in the Core imports an
