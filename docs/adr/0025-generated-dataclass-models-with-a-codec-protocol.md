...
([`docs/research/05`](../research/05-mattermost-rest-typing-codegen.md)). Hand-written models drift
from the spec. msgspec, the serialiser
[`docs/research/04`](../research/04-modern-python-library-engineering-2026.md) recommended for the
hot path, is a third-party library whose maintenance pace is outside our control, and a model type
that inherits from it puts every public type at its mercy. We decided:
...
- **Models are generated, never hand-maintained.** Inputs are versioned in the repository: the spec
  ...
  **committed**, regenerated idempotently, and checked in CI and pre-commit (`--check`), so a
  ...
  on a server release.
- **The model type is the standard library.** Generated models are `@dataclass(frozen=True,
  slots=True, kw_only=True)`: no third-party base class in any public type, understood by all four
  checkers without plugins ([ADR-0009](0009-four-strict-type-checkers.md)), and immune to the fate
  of any serialisation library. Hand-written models for what the spec leaves as `object` —
  `Post.props` well-known keys, attachments, dialog elements — use the same shape.
...
- **Serialisation is a Core-owned Protocol.** `Codec` — `encode(obj) -> bytes`, `decode(data, type)
  -> T`, `convert(obj, type) -> T`, strict types (no `str`→`int` coercion), unknown fields ignored —
  lives in the Core so generic plugins (State's `Flow` data, dedup keys) can serialise without
  knowing the implementation. The Adapter supplies **`MsgspecCodec` as the sole shipped
  implementation** and registers it as an App-scoped `Provider[Codec]`; msgspec is the Adapter's
  dependency, not the Core's ([ADR-0002](0002-core-scope-two-condition-test.md)). msgspec decodes
  dataclasses natively, honours `UNSET` on them, and its `dec_hook` unpacks Mattermost's
  JSON-in-JSON fields (`data.post`) into the generated `Post`. Replacing msgspec with pydantic's
  `TypeAdapter` is one module, not a major version.
...
- WebSocket payloads ([ADR-0012](0012-generic-event-envelope-with-adapter-payloads.md)) are
  dataclasses decoded by the same `Codec` and reuse the generated REST models; the full catalogue is
  #45.

## Considered options

- *`msgspec.Struct` as the model type* — rejected: fastest and most ergonomic, but every public type
  would inherit from a third-party library, and any change in its maintenance or wheel coverage
  would become a major version of ours.
...
- *`UNSET` everywhere, as the spec implies* — rejected: `str | Unset` on `post.id` in every handler
  contradicts [ADR-0006](0006-architectural-tenets-of-the-core.md).
