...
3.13 joins it. A new framework could start at 3.13 and stay standard-library-only, but 3.12 is still
the version most operating-system images and corporate base images ship, and the 0.5.0 audience
includes those environments. We decided **`requires-python = ">=3.12"`**, and to keep the typing
surface modern (PEP 696 TypeVar defaults, `TypeIs`, `ReadOnly`, `warnings.deprecated`) we admit
**`typing_extensions` as the single runtime dependency of the Core**, amending
[ADR-0002](0002-core-scope-two-condition-test.md)'s "standard library only" rule with exactly this
exception. It is maintained by the CPython typing council, has no transitive dependencies, and
disappears from the dependency list the day 3.12 is dropped.
...
- **Free-threading is a blocking job**: the suite runs on the free-threaded 3.14t build and must
  pass. Pure-Python code stays free-threading-safe by having no mutable module state
...
- *≥ 3.12 without the backport* — rejected: a public contract without `TypeIs`, TypeVar defaults and
  `ReadOnly` is weaker than the tenets in [ADR-0006](0006-architectural-tenets-of-the-core.md)
  demand.
