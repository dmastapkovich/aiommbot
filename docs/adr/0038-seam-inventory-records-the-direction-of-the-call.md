...
`ReplyChannel[R]` the Core's twelfth seam, and [§5.1.1](../design/05-building-block-view.md) has to
...
- **§5.1.1 carries a *Direction* column with UML's pair, `required` and `provided`.** A `required`
...
- **One table, not two.** UML keeps both directions on one diagram and Cockburn keeps both flavours
  ...
  row a section of its own, and §5.1.1's job is to be the single place a reader can count the
...
- **The *Specified in* column is authoritative, and §5 now says so itself.** ADR-0035 ruled that the
  ...
  correction in a third ADR, this one lifts ADR-0035's ruling into the reading guide of §5 and into
  §5.1.1 as the rule, so the prose that keeps being wrong stops being written. ADR-0035 counted six
  rows where the prose held and five where it did not; its conclusion is unchanged.
...
- *Keep `ReplyChannel` out of §5.1.1 as a part of `Event`* — rejected on cost, not on principle. It
  ...
  those projects all type the handed-out capability as a **concrete class**, which
...
- *Two blocks inside §5.1.1 instead of a column* — rejected: it is what the peer projects and the
  one published extension-point inventory do, but it makes a heading and a table for a single row,
  and the two doctrines with the most precise vocabulary both keep the two kinds in one picture.
