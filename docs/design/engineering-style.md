...
Every rule carries the seven parts [ADR-0033](../adr/0033-identified-tiered-rules-with-a-derived-review-checklist.md)
fixed:
...
| _Checked in_ | `LLD` — asked of a component design document. `PR` — asked of a diff. Most rules are both. |
| _From_ | The ADR or research note the rule descends from. |
...
equivalent. Everything else is a Plugin, an extra, or a documented recipe. The framework we are
replacing owned scheduling, metrics, retries, a breaker, storage profiles and a CLI, and usage
mining found most of that surface had no consumers at all.
...
it, and the two places inheritance remains the right tool — the error taxonomy and closed variant
...
which is how the 0.4.8 `Bot(Router)` came to own eight concerns.
...
_Limits:_ a component may still hold *parts* (§5.10 of the building-block view) — a part is not a
second reason to change, it is a named piece of the same one.
_Tier:_ `review`, with WPS module-member and method-count limits as the mechanical floor.
_Checked in:_ LLD, PR. _From:_ [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md), `docs/research/09`.
...
_Tier:_ `review`. _Checked in:_ LLD. _From:_ [ADR-0022](../adr/0022-state-plugin-model.md), `docs/research/07`.
...
_Tier:_ `tool` — the import-linter `layers`, `forbidden` and `independence` contracts.
_Checked in:_ LLD, PR. _From:_ [ADR-0032](../adr/0032-layer-model-and-direction-of-allowed-dependencies.md).
...
| [Chain of Responsibility](https://refactoring.guru/design-patterns/chain-of-responsibility) | the Inbound and Handler middleware layers | a list of hooks called in a fixed place |
...
| [Null Object](https://refactoring.guru/design-patterns/null-object) (via Prototype's family of "do nothing" objects) | the no-op observer default, disabled event isolation | `if observer is not None` at every call site |
...
|---|---|
| [Template Method](https://refactoring.guru/design-patterns/template-method) | inside one component it can be the honest shape of a fixed algorithm with one varying step; as a user extension point it is banned outright (§3.3) |
...
| [Proxy](https://refactoring.guru/design-patterns/proxy) | indistinguishable from Decorator at a glance, and the distinction matters for who owns lifetime |
...
<!-- Do -->
See ADR-00NN: an ABC inside this component, rejected alternative a Protocol plus a shared
...
_Tier:_ `review`. _Checked in:_ LLD. _From:_ [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md) (amended by #36).
...
| God object | the defect being replaced: eight concerns on `Bot(Router)` | WPS module members, methods, arguments |
| Deep inheritance and mixin soup | more than one level of framework class means the behaviour of a leaf cannot be read in one place | `review`, `ST-PAT-08` |
| Template Method as a user extension point | it makes every internal refactor a breaking change for users | `ST-TYP-08` (`@final`), `review` |
...
| Dictionary-as-record: a `data` bag, `**kwargs` payloads, tuple returns | a bag has no contract, so nobody can tell what a middleware is allowed to put in it | `semgrep:ST-TYP-12`, WPS110 |
...
_Tier:_ `review`. _Checked in:_ LLD. _From:_ [ADR-0002](../adr/0002-core-scope-two-condition-test.md), `docs/research/08`.

#### `ST-PAT-05` — Compose collaborators through the constructor and keep the class `@final`
...
# Do
bot = Bot(adapter=MattermostAdapter(...), plugins=[State(store=InMemoryStore())])
```

_Limits:_ a class whose document names who subclasses it and why is exempt — and must say so in its
public contract section.
_Tier:_ `tool` — `@final` plus four checkers. _Checked in:_ LLD, PR.
...
_From:_ [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md), `docs/research/04`.
...
_From:_ [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md) (amended by #36), [ADR-0032](../adr/0032-layer-model-and-direction-of-allowed-dependencies.md).
...
_Tier:_ `review`. _Checked in:_ LLD, PR. _From:_ [ADR-0029](../adr/0029-synchronous-face-from-a-sans-io-core-with-thin-drivers.md), [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md) (amended by #36).
...
_Tier:_ `review`. _Checked in:_ LLD. _From:_ `design-quality-checklist.md`.
...
guess: `Final = 30` freezes at `Literal[30]` and rejects the first duration that needs a fraction.
...
needs no `Final`.
_Tier:_ `review`. _Checked in:_ PR. _From:_ [ADR-0011](../adr/0011-lint-format-and-architecture-toolchain.md), and `ST-DOC-05` for the citation line.
...
_Tier:_ `tool` — ruff `UP`. _Checked in:_ PR. _From:_ [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md), [ADR-0008](../adr/0008-python-floor-3-12-with-typing-extensions.md).
...
_Tier:_ `tool` — ruff `ANN401` and `banned-api` on `typing.Any`, `semgrep:ST-TYP-03` for `Any` in
expressions. _Checked in:_ LLD, PR. _From:_ [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md), [ADR-0011](../adr/0011-lint-format-and-architecture-toolchain.md).
...
```python
# Don't
route = cast(Route, request.scope['route'])

# Do
routed = request.scope.get('route')
if isinstance(routed, Route):
    return routed.path
```

_Limits:_ no exceptions in `aiommbot/`; Quarantine may cast at the foreign boundary with a rule code
and an upstream link.
_Tier:_ `tool` — `banned-api` on `typing.cast`. _Checked in:_ LLD, PR. _From:_ [ADR-0010](../adr/0010-zero-suppressions-with-a-quarantine.md), [ADR-0011](../adr/0011-lint-format-and-architecture-toolchain.md).
...
_Tier:_ `tool` — slotscheck `require-subclass`, ruff. _Checked in:_ LLD, PR.
...
_Limits:_ none. `override` is a 3.12 name and comes from `typing`, not from the compat module.
_Tier:_ `tool` — mypy `explicit-override`, pyright `reportImplicitOverride`. _Checked in:_ PR.
...
_Limits:_ the error taxonomy's intermediate classes and the closed variant bases of `ST-PAT-07` are
open by design and say so in their contract section.
...
— come from `typing` as usual. The compat module is a Quarantine-adjacent module, not a Quarantine:
it holds no suppression.
_Tier:_ `tool` — `banned-api` lists the `typing` spelling of every name the compat module owns.
_Checked in:_ PR. _From:_ [ADR-0008](../adr/0008-python-floor-3-12-with-typing-extensions.md).

> Measured on the reference implementation: with `typing_extensions` imported directly, the same
> decision is spread over 62 import sites and only one name of ten (`TypedDict`) is mechanically
> protected — nothing stops a contributor importing the other nine from `typing` and breaking the
> floor build. That is the failure this rule and its `banned-api` list exist to prevent.
...
_From:_ [ADR-0008](../adr/0008-python-floor-3-12-with-typing-extensions.md), [ADR-0011](../adr/0011-lint-format-and-architecture-toolchain.md), `docs/research/04`.
...
def handle(event: 'Event[Post]') -> None: ...
...
_Tier:_ `tool` — the ruff `TC` family is off and `banned-api` forbids `typing.TYPE_CHECKING`.
_Checked in:_ LLD, PR. _From:_ [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md), [ADR-0011](../adr/0011-lint-format-and-architecture-toolchain.md).
...
# Do
async def handler(event: Event[Post], repo: TicketRepository) -> None: ...
```
...
_Tier:_ `tool` — ruff `ANN`, `semgrep:ST-TYP-12`, plus the check phase. _Checked in:_ LLD, PR.
_From:_ [ADR-0014](../adr/0014-filters-and-extractors-with-closed-handler-signatures.md), [ADR-0019](../adr/0019-handler-parameter-resolution-rules.md), `docs/research/03`.
...
def test_on_returns_decorator():
    assert callable(router.on(Post))

# Do — tests/typing/test_router.py, checked by all four checkers, never run by pytest
assert_type(router.on(Post), Callable[[MessageHandler], MessageHandler])

# Do — and the negative case is the point: this must not type-check
router.on(Post)(lambda event: None)   # type: ignore[arg-type]
```
...
_Checked in:_ LLD, PR. _From:_ [ADR-0009](../adr/0009-four-strict-type-checkers.md), [ADR-0010](../adr/0010-zero-suppressions-with-a-quarantine.md) (amended by #36).
...
value = client.thing  # type: ignore[attr-defined]  # upstream: encode/httpx2#123
```
...
inline.
_Tier:_ `tool` — `RUF100`, `warn_unused_ignores`, `reportUnnecessaryTypeIgnoreComment`, and the
baseline test. _Checked in:_ LLD, PR. _From:_ [ADR-0010](../adr/0010-zero-suppressions-with-a-quarantine.md) (amended by #36).
...
# Do
event = replace(event, meta=replace(event.meta, correlation_id=cid))
# or publish into Event scope, typed, per ADR-0020
```

_Limits:_ none in `aiommbot/`. A Quarantine module adapting a library that requires it says so and
counts the suppression.
_Tier:_ `tool` — `semgrep:ST-PAT-06` (dynamic attribute access) and `ST-TYP-14`. _Checked in:_ LLD, PR.
_From:_ [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md), [ADR-0010](../adr/0010-zero-suppressions-with-a-quarantine.md).

> Measured on the reference implementation: stashing framework state on the foreign request object
> (`request.__dmr_auth__`, `__dmr_endpoint__`, `__dmr_token__`, …) is the single largest source of
> its 100 `type: ignore` comments, almost all `[attr-defined]`.
...
_Tier:_ `tool` — ruff `ASYNC`, `semgrep:ST-ASY-01`. _Checked in:_ LLD, PR.
...
_Tier:_ `review`, with ruff `ASYNC` covering the mechanical cases. _Checked in:_ LLD, PR.
...
async for event in transport:                 # reader never stalls
    queue.put_nowait(event)                   # bounded; OverflowPolicy decides
...
async with asyncio.timeout(WORKER_DEADLINE):
...
own.
_Tier:_ `tool` — ruff `ASYNC`, `semgrep:ST-ASY-04`. _Checked in:_ LLD, PR.
_From:_ [ADR-0031](../adr/0031-stdlib-asyncio-with-a-fixed-concurrency-discipline.md).
...
# Do — inside the drain of the WebSocket gateway, and nowhere else
await asyncio.shield(self._finish_inflight())
```

_Limits:_ the drain of [ADR-0023](../adr/0023-websocket-gateway-resilience.md), bounded by its
25-second deadline. No other use.
_Tier:_ `tool` — `semgrep:ST-ASY-05` with a single allowed location. _Checked in:_ LLD, PR.
_From:_ [ADR-0031](../adr/0031-stdlib-asyncio-with-a-fixed-concurrency-discipline.md).
...
# Do
    ...
    raise unwrap_solitary(group) from None    # re-raises the group when it has siblings
```
...
# Do
@router.on(Post, sync_to_thread=True)
def handle(event: Event[Post]) -> None: ...
```
...
@router.on(Post, sync_to_thread=True)
def charge(event: Event[Post]) -> None:
...
#### `ST-ASY-10` — Declare Middleware, Signal subscribers, lifecycle hooks, observers and codecs as coroutine functions
...
#### `ST-ASY-12` — Send blocking work from inside an asynchronous Handler to `asyncio.to_thread`, not to the Bot's executor

_Reason:_ the Bot's executor is sized against the transport's worker count, so user work drawn from
the same budget starves dispatch.

```python
# Don't
await bot.executor.submit(render_pdf, doc)
...
def extract(self, event: Event[Post]) -> Ticket:
    raise NoMatchError

# Do
def extract(self, event: Event[Post]) -> Value[Ticket] | NoMatch | Invalid: ...
```
...
_Limits:_ no exceptions. The boundaries are named: the Dispatcher for the router walk, the
ErrorBoundary for an escaped exception, the check phase for start-up.
...
# Don't
for check in checks:
    if (failure := check()) is not None:
        raise StartupError(failure)

# Do
failures = [failure for check in checks if (failure := check()) is not None]
if failures:
    raise StartupError(failures)              # one raise, the full list
```
...
# Do
return CheckFailure(id='di.unknown-parameter', severity=Severity.error,
                    message=..., hint=...)
```
...
is broken — the defect measured in eight of eleven bots on the framework being replaced.
...
_Tier:_ `review`. _Checked in:_ LLD, PR. _From:_ [ADR-0021](../adr/0021-core-error-boundary.md), `docs/research/09`, `docs/research/12`.
...
_Limits:_ no exceptions. Field-level detail from `AppError` is allowed; bodies, headers, tokens,
message text and media are not.
...
#### `ST-ERR-09` — Root every exception at `AiommbotError`, suffix errors `…Error` and warnings `…Warning`

_Reason:_ one root lets an application catch everything the framework raises without catching
everything Python raises, and a suffix convention makes the taxonomy readable in a traceback.
...
_Tier:_ `tool` — ruff `N818` and pep8-naming. _Checked in:_ LLD, PR.
_From:_ [ADR-0027](../adr/0027-api-error-taxonomy.md), [ADR-0021](../adr/0021-core-error-boundary.md).

> A note on the shape: the reference implementation we measured keeps a flat hierarchy with no root
> class and answers "is this user-visible?" by the presence of a `status_code` attribute, which
> forces two hand-maintained tuples kept in step by `NOTE: keep this in sync` comments. A root
> class with the classification as a property removes that duty, which is why we take the other
> option here.
...
class ApiManager: ...       # the 0.4.8 name; the term is API client
...
_Tier:_ `review`. _Checked in:_ LLD, PR. _From:_ `documentation-style.md` §5, [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md).
...
_Tier:_ `review`. _Checked in:_ LLD, PR. _From:_ `CONTEXT.md`, and the three collisions #38 resolved
this way.
...
nothing else.
_Tier:_ `review`. _Checked in:_ LLD. _From:_ `documentation-style.md` §5, `docs/agents/domain.md`.
...
_Tier:_ `tool` — WPS110/111/117, with `allowed-domain-names` empty. _Checked in:_ PR.
...
_From:_ [ADR-0026](../adr/0026-standalone-typed-api-client-over-an-http-transport-protocol.md), [ADR-0029](../adr/0029-synchronous-face-from-a-sans-io-core-with-thin-drivers.md).
...
_Tier:_ `review`. _Checked in:_ LLD, PR. _From:_ `CONTEXT.md`, [ADR-0006](../adr/0006-architectural-tenets-of-the-core.md).
...
the two disagree.
_Tier:_ `review`. _Checked in:_ LLD. _From:_ `documentation-style.md` §4.
...
_Tier:_ `tool` — the public-surface guard test, plus `--verifytypes` at 100 %. _Checked in:_ LLD, PR.
_From:_ [ADR-0007](../adr/0007-tiny-public-root-with-explicit-subpackages.md) (amended by #36).
...
_Tier:_ `tool` — ruff `SLF001` and `PLC2701` package-wide, plus the import-linter contracts.
_Checked in:_ LLD, PR. _From:_ [ADR-0007](../adr/0007-tiny-public-root-with-explicit-subpackages.md), [ADR-0032](../adr/0032-layer-model-and-direction-of-allowed-dependencies.md).
...
# Do
from aiommbot import WebSocketConnection      # the seam the gateway is designed around
```

_Limits:_ inside one component, its own `_internal` modules import each other freely.
_Tier:_ `tool` — `PLC2701`, import-linter. _Checked in:_ LLD, PR.
...
`ST-MOD-10`.
_Tier:_ `tool` — import-linter `layers`, `forbidden`, `independence`. _Checked in:_ LLD, PR.
_From:_ [ADR-0032](../adr/0032-layer-model-and-direction-of-allowed-dependencies.md).
...
# Don't
ignore_imports =
  aiommbot.state -> aiommbot.mattermost

# Do
ignore_imports =
  # The default error reply needs the Adapter's message builder; the alternative
  # is a Core Protocol for one call site (see ADR-00NN).
  aiommbot.errors -> aiommbot.mattermost.builders
```
...
_Checked in:_ LLD, PR. _From:_ [ADR-0002](../adr/0002-core-scope-two-condition-test.md), [ADR-0015](../adr/0015-plugin-contract-and-composition.md), `docs/research/10`.
...
_From:_ [ADR-0015](../adr/0015-plugin-contract-and-composition.md), `docs/research/04`.
...
_Tier:_ `tool` — WPS module members and imports. _Checked in:_ LLD.
_From:_ [ADR-0011](../adr/0011-lint-format-and-architecture-toolchain.md), `documentation-style.md` §2.
...
is a Protocol.
_Tier:_ `tool` — import-linter `independence`. _Checked in:_ LLD, PR.
_From:_ [ADR-0032](../adr/0032-layer-model-and-direction-of-allowed-dependencies.md).
...
silence the gateway without silencing dispatch.
...
_Tier:_ `review`. _Checked in:_ LLD, PR. _From:_ `docs/research/04`, `docs/research/17`.
...
_Checked in:_ LLD, PR. _From:_ [ADR-0021](../adr/0021-core-error-boundary.md), [ADR-0024](../adr/0024-webhook-ingress-and-callback-security.md), [ADR-0026](../adr/0026-standalone-typed-api-client-over-an-http-transport-protocol.md), `docs/research/17`.
...
_Tier:_ `review`. _Checked in:_ LLD, PR. _From:_ [ADR-0026](../adr/0026-standalone-typed-api-client-over-an-http-transport-protocol.md), `docs/research/17`.
...
REDACTED_FIELDS: Final[frozenset[str]] = frozenset({
    'authorization', 'token', 'password', 'message', 'text', 'body', 'props', 'query',
})
```

_Limits:_ the list may only grow. Its exact membership is #29's to finalise with the observer
record; the rule is that there is exactly one list.
_Tier:_ `tool` — a test asserting no log record and no exception field intersects it.
_Checked in:_ LLD, PR. _From:_ [ADR-0026](../adr/0026-standalone-typed-api-client-over-an-http-transport-protocol.md), `docs/research/17`.
...
mechanism.
_Tier:_ `tool` — ruff `G` and `LOG`. _Checked in:_ PR. _From:_ `docs/research/04`.
...
logger.exception('handler failed', extra={'correlation_id': event.meta.correlation_id})
```
...
_Tier:_ `tool` — ruff `D1xx` with the Google convention. _Checked in:_ PR.
_From:_ [ADR-0011](../adr/0011-lint-format-and-architecture-toolchain.md), [ADR-0007](../adr/0007-tiny-public-root-with-explicit-subpackages.md) (amended by #36).
...
_Tier:_ `tool` — doctest collection over the package, `docstring-code-format`, and lint of docstring
code. _Checked in:_ LLD, PR. _From:_ #36 via `docs/research/04`, and the measured practice of the
reference implementation, which executes docstrings, `.rst` pages and its README alike.
...
_Tier:_ `review`, with ruff `ERA` and `TD` on the mechanical part. _Checked in:_ PR.
_From:_ `documentation-style.md` §8.
...
# Do
#: Mattermost's maximum `per_page`; the server silently clamps larger values
#: (`docs/research/05`).
PAGE_SIZE: Final[int] = 200
```
...
_Tier:_ `review`. _Checked in:_ PR. _From:_ `documentation-style.md` §8, and `ST-TYP-01`.
...
_Tier:_ `tool` — mypy's `deprecated` error code and the equivalents. _Checked in:_ PR.
_From:_ [ADR-0008](../adr/0008-python-floor-3-12-with-typing-extensions.md), `docs/research/04`.
...
or attributes.
_Tier:_ `tool` — `semgrep:ST-TST-01` restricting `mock`/`monkeypatch` to the allowed paths.
_Checked in:_ LLD, PR. _From:_ #36, evidenced by the reference implementation: two files in the
whole repository use `unittest.mock`, both at a third-party seam, against roughly eighteen
hand-written conforming doubles.
...
name.
_Tier:_ `review`. _Checked in:_ PR. _From:_ `design-quality-checklist.md`.
...
_Tier:_ `review`. _Checked in:_ LLD, PR. _From:_ `design-quality-checklist.md`, [ADR-0023](../adr/0023-websocket-gateway-resilience.md).
...
_Tier:_ `tool` — branch coverage at 100 %. _Checked in:_ PR.
_From:_ [ADR-0010](../adr/0010-zero-suppressions-with-a-quarantine.md), the quality bar of `docs/agents/context-brief.md`.
...
with pytest.warns(AiommbotWarning, match='sync_to_thread'):
    router.on(Post)(sync_handler)
```

_Limits:_ no exceptions; warnings are configured as errors for the whole suite.
_Tier:_ `tool` — `filterwarnings = ["error"]`. _Checked in:_ PR.
_From:_ [ADR-0030](../adr/0030-synchronous-callables-by-explicit-declaration.md).
...
_From:_ [ADR-0015](../adr/0015-plugin-contract-and-composition.md), `docs/research/03`.
...
must bound the wait.
_Tier:_ `tool` — a per-test timeout, plus `semgrep:ST-TST-09` on `asyncio.sleep` in tests.
_Checked in:_ LLD, PR. _From:_ [ADR-0022](../adr/0022-state-plugin-model.md), [ADR-0023](../adr/0023-websocket-gateway-resilience.md), `docs/research/13`.

## 12. Review checklist

Derived from the rules above and from nothing else. Every line names the rules it covers; a line
with no rule behind it does not belong here.

### 12.1 Design review — a component design document

Every rule tagged `LLD`, whatever its tier: at design time no tool has run.

- [ ] The component has one named reason to change; its parts are parts, not second responsibilities. `ST-SOL-01`, `ST-MOD-09`
...
- [ ] Every seam is a Protocol sized to one consumer, owned by the Core, with the conformance suite named. `ST-SOL-03`, `ST-SOL-04`, `ST-DOC-03`, `ST-TST-02`
...
- [ ] Collaborators arrive through the constructor, classes are `@final` unless the document names who subclasses them, no state is global, and inheritance appears only where §3 allows it. `ST-PAT-05`, `ST-PAT-06`, `ST-PAT-07`, `ST-PAT-08`, `ST-TYP-08`
- [ ] The contract has no `Any`, no `cast`, no `TYPE_CHECKING` import and no open signature; values crossing the seam are frozen; annotations are read through the one helper; state is never stashed on a foreign object. `ST-TYP-03`, `ST-TYP-04`, `ST-TYP-05`, `ST-TYP-10`, `ST-TYP-11`, `ST-TYP-12`, `ST-TYP-16`
...
- [ ] The colour of every callable the framework invokes is stated: synchronous Handlers and Providers are declared and idempotent, everything else is a coroutine function, and the loop belongs to the application. `ST-ASY-07`, `ST-ASY-08`, `ST-ASY-09`, `ST-ASY-10`, `ST-ASY-12`
...
- [ ] A bad composition stops the start with the full list; unexpected exceptions reach the ErrorBoundary; exceptions carry no content and are rooted at `AiommbotError` with `retryable` as a property. `ST-ERR-06` … `ST-ERR-10`
- [ ] Every name is its `CONTEXT.md` term, collides with no `_Avoid_` list, and any new concept gets its term in this commit; the `Sync` prefix and the type-parameter spellings match the decided ones. `ST-NAM-01`, `ST-NAM-02`, `ST-NAM-03`, `ST-NAM-05`, `ST-NAM-06`, `ST-NAM-07`, `ST-NAM-08`
...
- [ ] Public names are documented with executable examples, and Protocol docstrings state the implementer's contract. `ST-DOC-01`, `ST-DOC-02`, `ST-DOC-03`
- [ ] The testing section names the doubles it uses (never mocks of our own types), the conformance suite, a test per failure mode, and how time is controlled. `ST-TST-01`, `ST-TST-02`, `ST-TST-04`, `ST-TST-09`

### 12.2 Code review — a pull request

Only `review`-tier rules. Everything else in this document is already red in CI, and re-checking it
by hand is how a checklist stops being read.
...
- [ ] Any pattern introduced is named with its rejected alternative; a restricted pattern brought its ADR; the diff introduces no banned pattern. `ST-PAT-01`, `ST-PAT-02`, `ST-PAT-03`
- [ ] New Protocols are sized to one consumer; new classes are `@final` unless documented otherwise; new shared behaviour is not a mixin outside a private async/sync base. `ST-SOL-04`, `ST-PAT-07`, `ST-PAT-08`, `ST-TYP-08`
...
- [ ] No timeout wraps `__anext__`; a bounded queue sits between producer and consumer; `shield` appears only in the drain; exception groups keep their siblings; the loop is still the application's. `ST-ASY-03`, `ST-ASY-05`, `ST-ASY-06`, `ST-ASY-07`
...
- [ ] Every new failure is an outcome or an exception by the rule, converted at a boundary, with one representation and no payload; retryability is read, not recomputed. `ST-ERR-01`, `ST-ERR-02`, `ST-ERR-03`, `ST-ERR-05`, `ST-ERR-08`, `ST-ERR-10`
- [ ] No handler swallows an exception it did not expect. `ST-ERR-07`
- [ ] Names are glossary terms, collide with no `_Avoid_` list, and any new concept arrives with its term in this commit. `ST-NAM-01`, `ST-NAM-02`, `ST-NAM-03`, `ST-NAM-06`, `ST-NAM-07`, `ST-NAM-08`
...
- [ ] A Protocol docstring changed with its contract, and the conformance suite it names still applies. `ST-DOC-03`, `ST-TST-02`
