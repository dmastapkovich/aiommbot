...
5. **Changing a section's number is a sweep, not an edit.** A cross-reference to a numbered section
   ...
   template's §10, `Chapter 8` and `Chapter 5` in its published examples. Its FAQ does ask for a
   hyperlink where a reader is being sent to detail (C-4-3, C-4-2), which is why a *Detail in*
   column links; a reference in running prose stays bare. So nothing mechanical sees a bare
   reference go stale. A ticket that renumbers a heading therefore greps `§<old number>` over
   every tracked Markdown file and repairs it **in the same commit**,
...
| 11 Risks and technical debt | **"ordered by priority"** is the whole of it — stated once, in the template's Contents, and never defined: not one of the six tips mentions priority at all, and no page says how to rank. Form is "**List** of risks and/or technical debts, **probably** including suggested measures"; the German original says "Liste **oder Tabelle**" and the section page's placeholder permits either, so a table is sanctioned and a list is the default. Owner, date, probability, impact and severity appear **nowhere** — not in the template's twelve languages, the section page, the six tips or FAQ C-11-1, the only FAQ question of 136 in this category, which collapses risk and debt into "the currently known problems" and licenses a row to point at "source code, structures, concepts, **decisions** and all other aspects of the system". The six tips are all about how to *find* risks, never how to write one down | No count anywhere, and FAQ B-1 runs the other way: "Please don't fill in everything. Document only what your stakeholders need". Published practice is short and loose — 8 documents, median 29.5 lines, range 8–46; 4 prose, 2 bullet lists, 2 tables; **1 of 8** orders visibly and 2 define a scale, by two different methods; **0 of 8** carry an owner, a date, a numeric score or a diagram; 5 of 8 say in their own words that the section is thin. The one concrete risk type the corpus agrees on is somebody else's code — a third-party dependency, tool version or unmaintained component, in **6 of 8** | in-session, from `.refs/arc42` |
...
**§3 has no row**, so step 1 applies to it. §3 was written before the measuring started, which is a
reason to measure before *changing* it, not a reason to treat it as measured.
